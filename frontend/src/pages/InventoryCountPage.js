              ...
              <div className="space-y-6">
                ...
                >
                  ...
                  <p className="text-lg font-medium mb-2">
                    {language === 'ar' ? 'انقر لتحميل ملف Excel' : 'Cliquez pour charger un fichier Excel'}
                  </p>
                  <p className="text-sm text-muted-foreground">
                    {language === 'ar' ? 'أو اسحب الملف هنا (xlsx, xls)' : 'Ou glissez le fichier ici (xlsx, xls)'}
                  </p>
                  <input
                    ref={fileInputRef}
                    type="file"
                ...
                <div className="flex justify-center">
                  <Button variant="outline" onClick={downloadTemplate} className="gap-2">
                    ...
                    {language === 'ar' ? 'تحميل قالب Excel' : 'Télécharger le modèle'}
                  </Button>
                </div>

                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <h4 className="font-medium text-blue-800 mb-2">
                    {language === 'ar' ? 'تنسيق الملف المطلوب:' : 'Format de fichier requis:'}
                  </h4>
                  <ul className="text-sm text-blue-700 space-y-1">
                    ...
                    <li>• {language === 'ar' ? 'عمود الاسم (اختياري - للعرض فقط)' : 'Colonne nom (optionnel - pour affichage)'}</li>
                  </ul>
                </div>
              </div>
            )}
            ...
            {importStep === 2 && (
              <div className="space-y-6">
                <div className="bg-muted p-4 rounded-lg">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {importData.length} {language === 'ar' ? 'صف' : 'lignes'}
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <Select value={importMapping.barcode} onValueChange={(v) => setImportMapping(m => ({ ...m, barcode: v }))}>
                      <SelectTrigger className="mt-1">
                        <SelectValue placeholder={language === 'ar' ? 'اختر العمود' : 'Sélectionner'} />
                      </SelectTrigger>
                      <SelectContent>
                    ...
                    <Table>
                        ...
                        <TableRow>
                          {excelColumns.map(col => (
                            <TableHead key={col.index} className={
                              importMapping.barcode === col.index.toString() ? 'bg-blue-100' :
                              importMapping.quantity === col.index.toString() ? 'bg-green-100' :
                              importMapping.name === col.index.toString() ? 'bg-amber-100' : ''
                            }>
                              {col.name}
                            </TableHead>
                          ))}
                      ...
                      <TableBody>
                        {importData.slice(0, 5).map((row, idx) => (
                          <TableRow key={idx}>
                            {excelColumns.map(col => (
                              <TableCell key={col.index} className={
                                importMapping.barcode === col.index.toString() ? 'bg-blue-50' :
                                importMapping.quantity === col.index.toString() ? 'bg-green-50' :
                                importMapping.name === col.index.toString() ? 'bg-amber-50' : ''
                              }>
                                {row[col.index] ?? ''}
                              </TableCell>
                            ))}
                ...
                <div className="flex gap-2 justify-between">
                  <Button variant="outline" onClick={resetImport}>
                    ...
                    {language === 'ar' ? 'إعادة' : 'Recommencer'}
                  </Button>
                  <Button onClick={generateImportPreview} disabled={!importMapping.barcode || !importMapping.quantity}>
                    {language === 'ar' ? 'معاينة التغييرات' : 'Aperçu des changements'}
                  </Button>
                </div>
              </div>
            )}
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="p-4 bg-green-50 rounded-lg text-center">
                    <p className="text-2xl font-bold text-green-600">
                      {importPreview.filter(p => p.status === 'matched').length}
                    </p>
                    <p className="text-sm text-green-600">{language === 'ar' ? 'متطابق' : 'Trouvés'}</p>
                  </div>
                  <div className="p-4 bg-red-50 rounded-lg text-center">
                    <p className="text-2xl font-bold text-red-600">
                      {importPreview.filter(p => p.status === 'not_found').length}
                    </p>
                    <p className="text-sm text-red-600">{language === 'ar' ? 'غير موجود' : 'Non trouvés'}</p>
                  </div>
                  <div className="p-4 bg-amber-50 rounded-lg text-center">
                    <p className="text-2xl font-bold text-amber-600">
                      {importPreview.filter(p => p.status === 'matched' && p.diff !== 0).length}
                    </p>
                    <p className="text-sm text-amber-600">{language === 'ar' ? 'فروقات' : 'Différences'}</p>
                  </div>
                </div>
                    ...
                    <TableBody>
                      {importPreview.map((item, idx) => (
                        <TableRow key={idx} className={item.status === 'not_found' ? 'bg-red-50' : item.diff !== 0 ? 'bg-amber-50' : ''}>
                          ...
                          <TableCell>
                            {item.productName || item.excelName || '-'}
                          </TableCell>
                          <TableCell className="text-center">
                            {item.currentQty ?? '-'}
                          </TableCell>
                          ...
                          <TableCell className="text-center">
                            {item.diff !== null && (
                              <Badge className={
                                item.diff === 0 ? 'bg-green-100 text-green-700' :
                                item.diff > 0 ? 'bg-blue-100 text-blue-700' : 'bg-red-100 text-red-700'
                              }>
                ...
                <div className="flex gap-2 justify-between">
                  ...
                  <Button 
                    ...
                    disabled={importPreview.filter(p => p.status === 'matched').length === 0}
                  >
                    <Check className="h-4 w-4 me-2" />
                    {language === 'ar' ? 'تطبيق البيانات' : 'Appliquer les données'}
                    ({importPreview.filter(p => p.status === 'matched').length})
                  </Button>
                </div>
              </div>
